...
## 7a. Validation, and what the backtest actually shows

APIx is compared against a **reference average-fare series** over the full
35-day window (`index/backtest.py`).

> **The reference is a synthetic stand-in, not DGCA data.** DGCA's portal serves
> a JavaScript shell with no machine-readable fare series, and its traffic
> material is published as *monthly* PDFs — while APIx here is a *daily* series.
> No real DGCA daily average-fare series exists to validate against. The
> attempt and the reasoning are recorded in `config/backtest.yaml`.

The reference is deliberately **not** APIx with noise added — that would give a
meaningless correlation. It is computed the way a simple published average fare
actually is: an **unweighted mean of every observed fare**, rebased to 100.
...
The two series agree closely in *shape* but diverge in *level* — and that
divergence is not an error. It is the composition effect APIx is built to
remove, and it is measurable:

| Booking window | Share of rows | Index weight | Over-weighted by | Rows on festival dates |
|---|---|---|---|---|
| 0–3 days | 6.0% | 0.12 | 0.50× | 0.0% |
| 4–7 days | 8.5% | 0.18 | 0.47× | 0.0% |
| 8–14 days | 15.4% | 0.24 | 0.64× | 0.0% |
| 15–21 days | 15.7% | 0.20 | 0.78× | 0.0% |
| 22–30 days | 20.3% | 0.16 | 1.27× | 8.0% |
| **31–45 days** | **34.1%** | **0.10** | **3.41×** | **22.8%** |

A naive average counts each observation once, so the 31–45 day window supplies
34% of its input while economically deserving 10%. And that is precisely the
window whose travel dates fall on the October festivals — 22.8% of those rows,
against 0% for every window inside three weeks.

The consequence, measured on the same data:
